/*
    ...
    Requerimiento 6: Generar el Program.cs invocando la primera produccion 
*/
using System.IO;
using System;
namespace Generador
{
    public class Lenguaje:Sintaxis
    {
        ...
        private void Produccion(bool Primero=true)
        {
            if(Primero)
            {
                // GeneraProgram(getContenido()); 
                Genera("public void " + getContenido()+("()"));
            }
            else
            {
                Genera("private void " + getContenido()+("()")); //buscar donde es falso
            }
            Genera("{");
            Match(Tipos.SNT);
        ...
        private void ListaORs(bool Primero=true) 
        {
            string Simbolo=getContenido();
            Match(Tipos.ST);
            if(EsClassificacion(Simbolo))
            {
                ...
                {
                    Genera("else if(getClasificacion()==Tipos."+ "\""+ Simbolo +"\");");
                }
            }
            else
            {
                ...
                {
                    Genera("else if(getContenido()=="+ "\""+ Simbolo +"\");");
                }
            }
            Genera("{"); 
            if(EsClassificacion(Simbolo))
